...
if (smiley_wysiwyg)
{
       ...
       }*///doan nay se thiet lap smiley_path ngay tu server de tranh truong hop ko dung chung voi mod seo dc
c=smiley_code.length;
var temp1,temp2;
for(i=0;i<c;i++)
	for(j=i;j<c;j++)
		if(smiley_code[i].length<smiley_code[j].length)
		{
			temp1=smiley_code[i];
			temp2=smiley_img[i];
			smiley_code[i]=smiley_code[j];
			smiley_img[i]=smiley_img[j];
			smiley_code[j]=temp1;
			smiley_img[j]=temp2;
		}
       for(var i = 0; i < smiley_total; i++) {
		...
		function replace_from(str,search_str,replace_str,position)
		{
			str1=str.substr(0,position);
			str2=str.substr(position);
			str2=str2.replace(search_str,replace_str);
			str=str1+str2;
			return str;
		}
       function Smiley_BBcodetoHTML(str) {
          for(var i = 0; i < smiley_total; i++) {
			 smiley_pos=0;
             while ((smiley_pos=str.indexOf(smiley_code_bbcode[i],smiley_pos-1)) > -1) {
				bchar=str.substr(smiley_pos-1,1);
				if(smiley_pos-1<0 || bchar==' ' || bchar=='.')
					str = replace_from(str,smiley_code_bbcode[i], smiley_img_bbcode[i],smiley_pos);
				else smiley_pos=str.indexOf(smiley_code_bbcode[i],smiley_pos)+smiley_code_bbcode[i].length;
				if(str.indexOf(smiley_img_bbcode[i],smiley_pos) > -1)
					smiley_pos=str.indexOf(smiley_img_bbcode[i],smiley_pos)+smiley_img_bbcode[i].length;
             }
          }
          return str;
       };
       ...
       function Smiley_HTMLtoBBcode(str) {
          for(var i = 0; i < smiley_total; i++) {
			 smiley_bbcode_pos=0;
             while (str.indexOf(smiley_img_bbcode[i],smiley_bbcode_pos) > -1) {
                str = str.replace(smiley_img_bbcode[i], smiley_code_bbcode[i]);
				smiley_bbcode_pos=str.indexOf(smiley_code_bbcode[i],smiley_bbcode_pos)+smiley_code_bbcode;
             }
          }
          return str;
       };
...
_BBcodetoHTML : function (a)
{
    function r(re, str) {
       ...
       // r(/\[\/(sub|sup|strike|s|b|i|u|blockquote)\]/gi,'</$1>');
		r(/\[i\](.*?)\[\/i\]/gi,'<i>$1</i>');
		r(/\[sub\](.*?)\[\/sub\]/gi,'<sub>$1</sub>');
		r(/\[sup\](.*?)\[\/sup\]/gi,'<sup>$1</sup>');
		r(/\[strike\](.*?)\[\/strike\]/gi,'<strike>$1</strike>');
		r(/\[b\](.*?)\[\/b\]/gi,'<b>$1</b>');
		r(/\[s\](.*?)\[\/s\]/gi,'<s>$1</s>');
		r(/\[u\](.*?)\[\/u\]/gi,'<u>$1</u>');
		/*r(/\[blockquote\](.*?)\[\/blockquote\]/gi,'<blockquote>$1</blockquote>');*/
        r(/\[font=(.*?)\]/gi,'<font face="$1">');
        r(/\[color=(.*?)\]/gi,'<font color="$1">');
        r(/\[highlight=(.*?)\]/gi,'<span style="background-color:$1">');
        r(/\[\/highlight\]/gi,'</span>');      
        r(/\[size=(.*?)\]/gi,'<font style="font-size:$1%">');
...
_HTMLtoBBcode : function (a) {
        ...
        if (b.length > 1) {
            for (var i = 0; i < b.length; i++) {
                ...
                var f = b[i];
                if (f.match(/<(div|span|font|strong|b|u|i|em|var|address|h1|h2|h3|h4|h5|h6|img|ol|ul|li|a|strike|s|sub|sup|hr|table|tr|td)( ([^>]{1,}.*?)){0,1}( {0,1}){0,1}>/i)) {
                    var g = RegExp.$1;
                    var h = RegExp.$3;
                        ...
                        if (j.length > 0) {
                            for (var r in k) {
                                if (r == "color") {
									...
									if(k.size==1) m += "[size=85]";
									else if(k.size==2) m += "[size=100]";
									else m += "[size=" + k.size*40 + "]";
                                } else if (r == "face") {
                                    m += "[font=" + k.face + "]";
                                } else if (r == "style") {
                                    m += this._process(g, k);
                ...
                } else if (f.match(/<\/(div|span|font|strong|b|u|i|em|var|address|h1|h2|h3|h4|h5|h6|ol|ul|li|a|strike|s|sub|sup|table|tr|td)>/i)) {
                    e--;
                    var w = c.pop();
                    if (w != null) {
                        var x = "";
		...
		if(d[j]!=""&&d[j]!=null){
			...
			if(e[1]){
				var g=e[1].replace(/^ +| +$/g,"");
					...
					sizetype=g.substr((parseInt(g)+'').length,g.length);
					if(sizetype=="%") c +="[size="+parseInt(g)+"]";
					else if(sizetype=="pt"){
...
_HTML : function (tag,tagend,a)
        ...
        if(isIE)
        {
                ...
                c.document.execCommand('insertHTML',false,cc);
				//var e=c.document.body.innerHTML;
				//e=e.replace(/src="([\.\/]+)\/images\/smilies\/([\w-\.]+)"/gi,"src=\""+v+smiley_path+"$2\"");
				//c.document.body.innerHTML=e;
                c.document.execCommand("removeformat",false,"");
                c.focus();
                }
        }
...
function AddSmileyIcon(imagePath,theme){
        // removing dot from URL
                 if (comm._viewmode == 1){
				imagePath = site_full_url + imagePath.replace(/.*\/images\/smilies/gi,"/images/smilies");
				//var Editor = document.getElementById('box').contentWindow.document;
                comm._focus();
				...
				else
				{
					...
					Editor.queryCommandEnabled("InsertImage");
					Editor.execCommand("InsertImage", false,'#jwysiwyg#');
					elements=Editor.getElementsByTagName("img");
					for(i=0;i<elements.length;i++)
					{
						var value=elements[i].getAttribute("src");
						//if(isIE) value=value.substr(value.length-10);
						if(value=='#jwysiwyg#'){
